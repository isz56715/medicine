...
import 'CP004_provider.dart';

class CP004Controller extends GetxController with StateMixin<dynamic> {
  ...
  void onInit() {
    super.onInit();
    search.value = Get.arguments["search"];
    pdate.value = Get.arguments["pdate"];
    comment.value = Get.arguments["comment"];
    getCP004();
    print(search.value);
  }
  ...
  // select
  void getCP004() {
    CP004Provider().getCP004(search.value).then((value) {
  ...
  Future<void> clickButtonDelete(mnum) async {
    CP004Provider().setCP003Delete(mnum).then((value) {
      print(value);
...
// insert
  Future<void> clickButtonInsert(mnum, pdate) async {
    ...
    print(parms);
    CP004Provider().setCP003Insert(parms).then((value) {
      print(value);
    }, onError: (erorr) {
      print(erorr);
      Get.defaultDialog(
